...
from flownet3d_util import PointNetSetAbstraction, PointNetFeaturePropogation, FlowEmbedding
...
class Flow0Regressor(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=32, in_channel=128,
                                          mlp=[128, 128, 128],
...
class FlowRegressor(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=32, in_channel=128,
                                          mlp=[128, 128, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=32, in_channel=128,
                                          mlp=[128, 128, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.fc = torch.nn.Linear(128, 3)
...
class GlobalCorrLayer(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 16), radius=None, nsample=16, in_channel=3,
                                          mlp=[32, 32, 64],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.fp1 = PointNetFeaturePropogation(in_channel=64, mlp=[])
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 8), radius=None, nsample=16, in_channel=64,
                                          mlp=[64, 64, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
    ...
    def forward(self, pc1_l_glob, pc2_l_glob, feats1_glob, feats2_glob):
        ...
        flow0 = (corr_mat @ pcloud2.permute(0, 2, 1).contiguous()) / (row_sum + 1e-8) - pcloud1.permute(0, 2,
                                                                                                        1).contiguous()
...
class EncoderLoc(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 2), radius=None, nsample=32, in_channel=3,
                                          mlp=[32, 32, 32],
                                          group_all=False, return_fps=True, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=32, in_channel=32,
                                          mlp=[64, 64, 64],
                                          group_all=False, return_fps=True, use_instance_norm=use_instance_norm)
...
class EncoderGlob(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 8), radius=None, nsample=32, in_channel=64,
                                          mlp=[128, 128, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 16), radius=None, nsample=24, in_channel=128,
                                          mlp=[128, 128, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.sa3 = PointNetSetAbstraction(npoint=int(npoint / 32), radius=None, nsample=16, in_channel=128,
                                          mlp=[256, 256, 256],
                                          group_all=False, use_instance_norm=use_instance_norm)
...
class H0Net(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=8, in_channel=64,
                                          mlp=[128, 128, 128],
                                          group_all=False, use_instance_norm=use_instance_norm)
...
class FlowStep3D(nn.Module):
    def __init__(self, npoint=8192, use_instance_norm=False, loc_flow_nn=32, loc_flow_rad=1.5, k_decay_fact=1.0,
        ...
        self.local_corr_layer = FlowEmbedding(radius=loc_flow_rad, nsample=loc_flow_nn, in_channel=64,
                                              mlp=[128, 128, 128],
        ...
        self.flow_conv1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=3,
                                                 mlp=[32, 32, 32],
                                                 group_all=False, use_instance_norm=use_instance_norm)
        self.flow_conv2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=8, in_channel=32,
                                                 mlp=[16, 16, 16],
                                                 group_all=False, use_instance_norm=use_instance_norm)
...
def sequence_loss(pos1, pos2, flows_pred, flow_gt):
    ...
    for i, w in enumerate(loss_iters_w):
        loss += w * torch.nn.L1Loss()(flows_pred[i], flow_gt)
    return loss
...
def fore_back_loss(pc1, pc2, flow, ego_flow_gt, label1, label2, clusters_label1, clusters_label2):
    ...
    for b_idx in range(B):
        ...
        bg_flow_curr = torch.unsqueeze(flow_curr[:, bg_idx1_curr], 0)

        # -------------- TransformationLoss -------------------------------------------------
        trans_loss += l1_loss(bg_pc1_curr + bg_flow_curr, bg_pc1_curr + bg_ego_flow_gt_curr)
    ...
    trans_loss /= B
    smoothness_loss /= B
    return trans_loss, chamfer_loss, smoothness_loss

def self_flowstep3d_sequence_loss(pos1, pos2, flows_pred, ego_flow_gt, label1, label2, clusters_foreground_label1, clusters_foreground_label2):
    # loss_iters_w = [0.7, 0.4, 0.4, 0.4]
    ...
    smoothness_loss_sum = torch.zeros(1).cuda()

    for i in range(len(flows_pred)):
        w = loss_iters_w[i]

        trans_loss, chamfer_loss, smoothness_loss = fore_back_loss(pos1, pos2,
                                                                    flows_pred[i],
                                                                    ego_flow_gt,
                                                                    label1,
                                                                    label2,
