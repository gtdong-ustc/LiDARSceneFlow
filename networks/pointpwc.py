...
def multiScaleChamferSmoothCurvature(pc1, pc2, pred_flows):
    ...
    for i in range(num_scale):
        ...
        dist1 = dist1.unsqueeze(1)
        dist2 = dist2.unsqueeze(1)
        print(dist1.shape)
...
def ChamferSmoothCurvatureLoss(cur_pc1, cur_pc2, cur_flow, cur_weights, cur_label1, cur_label2):
    """

    :param cur_pc1: (torch.Tensor): (B, M, 3) points
    :param cur_pc2: (torch.Tensor): (B, M, 3) points
    :param cur_flow: (torch.Tensor): (B, M, 3) flow
    :param cur_weights: (torch.Tensor): (B, M, 1)
    :return:
    """
    f_curvature = 0.3
    f_smoothness = 1.0
    ...
    cur_pc1 = cur_pc1.permute(0, 2, 1)  # : (B,3,N)
    cur_pc2 = cur_pc2.permute(0, 2, 1)  # : (B,3,N)
    cur_flow = cur_flow.permute(0, 2, 1)  # pred_flow: (B,3,N)
    cur_weights = cur_weights.permute(0, 2, 1) #(B,1,N)
    cur_label1 = cur_label1.permute(0,2,1)#(B,1,N)
    cur_label2 = cur_label2.permute(0, 2, 1)  # (B,1,N)

    # weights_normalized = cur_weights / (torch.sum(cur_weights, dim=2, keepdim=True) + _EPS)
    ...
    dist1 = dist1 * cur_weights * cur_label1
    # dist2 = dist2 * cur_label2
    chamferLoss = dist1.sum(dim=1).mean() #+ dist2.sum(dim=1).mean()

    # smoothness
    smoothnessLoss = computeSmooth(cur_pc1, cur_flow) * cur_label1
    smoothnessLoss = smoothnessLoss.sum(dim=1).mean()

    # curvature
    inter_pc2_curvature = interpolateCurvature(cur_pc1_warp, cur_pc2, cur_pc2_curvature)
    curvatureLoss = (inter_pc2_curvature - moved_pc1_curvature) * cur_weights.permute(0,2,1) * cur_label1.permute(0,2,1)
    curvatureLoss = torch.sum(curvatureLoss ** 2, dim=2).sum(dim=1).mean()
    # curvatureLoss = torch.sum((inter_pc2_curvature - moved_pc1_curvature) ** 2, dim=2).sum(dim=1).mean()
    cur_loss = f_chamfer * chamferLoss + f_curvature * curvatureLoss + f_smoothness * smoothnessLoss

    # print(chamferLoss)
    # print(smoothnessLoss)
    # print(curvatureLoss)

    return cur_loss, chamferLoss, curvatureLoss, smoothnessLoss

def FGChamferSmoothCurvatureLoss(cur_pc1, cur_pc2, cur_flow, cur_weights):
    """
    ...
    # curvature
    inter_pc2_curvature = interpolateCurvature(cur_pc1_warp, cur_pc2, cur_pc2_curvature)
    curvatureLoss = torch.sum(((inter_pc2_curvature - moved_pc1_curvature) * cur_weights.permute(0,2,1)) ** 2, dim=2).sum(dim=1).mean()
    # curvatureLoss = torch.sum((inter_pc2_curvature - moved_pc1_curvature) ** 2, dim=2).sum(dim=1).mean()
    cur_loss = f_chamfer * chamferLoss + f_curvature * curvatureLoss + f_smoothness * smoothnessLoss
...
def SmoothCurvatureLoss(cur_pc1, cur_pc2, cur_flow, cur_weights):
    """
    ...
    cur_loss = f_curvature * curvatureLoss + f_smoothness * smoothnessLoss

    return cur_loss, curvatureLoss, smoothnessLoss

def ChamferLoss(cur_pc1, cur_pc2, cur_flow, cur_weights):
    """
    ...
    # weights_normalized = cur_weights / (torch.sum(cur_weights, dim=2, keepdim=True) + _EPS)
    weights_normalized = cur_weights.squeeze(1)
    # print(weights_normalized.max())
    # print(weights_normalized.min())

    # compute curvature
    cur_pc1_warp = cur_pc1 + cur_flow
    dist1, dist2 = computeChamfer(cur_pc1_warp, cur_pc2)
    chamferLoss = (dist1*weights_normalized).sum(dim=1).mean() + (dist2*weights_normalized).sum(dim=1).mean()

    # smoothness
    # print(computeSmooth(cur_pc1, cur_flow).shape)

    cur_loss = f_chamfer * chamferLoss

    return cur_loss

def ChamferSmoothLoss(cur_pc1, cur_pc2, cur_flow, cur_weights):
    """
    ...
    cur_pc1_warp = cur_pc1 + cur_flow
    dist1, dist2 = computeChamfer(cur_pc1_warp, cur_pc2)
    chamferLoss = (dist1 * cur_weights).sum(dim=1).mean() # + (dist2 * cur_weights).sum(dim=1).mean()

    # smoothness
    smoothnessLoss = computeSmooth(cur_pc1, cur_flow).sum(dim=1).mean()

    cur_loss = f_chamfer * chamferLoss + f_smoothness * smoothnessLoss
