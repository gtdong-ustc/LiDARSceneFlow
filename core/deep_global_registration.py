...
class DeepGlobalRegistration:
  ...
  def preprocess(self, pcd):
    ...
    ticks = time.time()
    # result_path = '/home/ashesknight/Documents/result/DeepGlobalRegistration/output/' + 'sel_' + str(ticks) + '.npy'
    result_path = '/output/' + 'sel_' + str(ticks) + '.npy'
    np.save(result_path, sel)
    print('voxel_size {}'.format(self.voxel_size))
    print('npts {}'.format(npts))
    print('sel sample 0: {}, 1: {}, 10: {}, 20: {}'.format(sel[0], sel[1], sel[10], sel[20]))
    print('sel shape {}, sel type {}'.format(sel.shape, type(sel)))
    ...
    print('corrds shape {}'.format(coords.shape))
  ...
  def register(self, xyz0, xyz1, inlier_thr=0.00):
    ...
    with torch.no_grad():
      ...
      print("corres_idx0 shape {}, corres_idx1 shape {}".format(coords0.shape, coords1.shape))
      ...
      print("inlier_coords shape {}".format(inlier_coords.shape))
    ...
    if wsum >= wsum_threshold:
      ...
      # > Case 1: Safeguard RANSAC + (Optional) ICP0
