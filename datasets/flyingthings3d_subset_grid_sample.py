...
class FlyingThings3DSubset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pc1_norm = pc1_transformed[:self.num_points,:]
        pc2_norm = pc2_transformed[:self.num_points,:]
        return pc1_transformed, pc2_transformed, pc1_norm, pc2_norm, sf_loaded, self.samples[index]
    ...
    def make_dataset(self, full):
        ...
        root = osp.join(root, 'train_sample') if self.train else osp.join(root, 'val_sample')
    ...
    def pc_loader(self, path):
        ...
        gt = np.load(osp.join(path, 'gt.npy'))
        pc1[..., -1] *= -1
        pc2[..., -1] *= -1
        gt[...,-1] *= -1
        pc1[..., 0] *= -1
