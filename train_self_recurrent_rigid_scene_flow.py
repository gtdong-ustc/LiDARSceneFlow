...
def main():
    ...
    # val_dataset = datasets.__dict__['WaymoOpen'](
    #     train=False,
    #     transform=transforms.ProcessDataWaymo(args.data_process,
    #                                           args.num_points,
    #                                           args.allow_less_points),
    #     num_points=args.num_points,
    #     data_root=args.data_root
    ...
    if args.pretrain is not None:
        ...
        if args.multi_gpu is not None:
            ...
            for k, v in pretrained_dict.items():
                ...
                # del (k_split[0])
                # if k_split[0] == 'encoder_loc' or k_split[0] == 'encoder_glob':
                #     new_k = ".".join(k_split)
                #     new_k = 'module.' + new_k
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            if args.model_name == 'SequenceWeights':
                ...
                # loss, trans_loss, inlier_loss, chamfer_loss, rigid_loss = sequence_loss(pos1, pos2, pred_flows, pred_rigid_flows, ego_flow, gt_label1, gt_label2, pred_weights)
                ...
                # print(clusters_foreground_label1.sum())

                pred_flows = model(pos1, pos2, norm1, norm2, _ITE_NUM)
                # print(pred_flows[-1].shape)
            ...
            # total_trans_loss += trans_loss.cpu().data * args.batch_size
            # total_inlier_loss += inlier_loss.cpu().data * args.batch_size
            # total_chamfer_loss += chamfer_loss.cpu().data * args.batch_size
            # total_rigid_loss += rigid_loss * args.batch_size
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            if args.model_name == 'SequenceWeights':
                ...
                # clusters_1 = get_instance_cluster_gt(fg_label1)
                # clusters_2 = get_instance_cluster_gt(fg_label2)
                # clusters_foreground_label1 = get_clusters_fore_label(pc1, clusters_1).permute(0, 2, 1)
                # clusters_foreground_label2 = get_clusters_fore_label(pc2, clusters_2).permute(0, 2, 1)
                #
                ...
                # label1_path = os.path.join(root_dir, 'label1_'+filename)
                # label2_path = os.path.join(root_dir, 'label2_'+filename)
                # clusters_foreground_label1_np = clusters_foreground_label1[0].cpu().numpy()
                # clusters_foreground_label2_np = clusters_foreground_label2[0].cpu().numpy()
                #
                # np.save(label1_path, clusters_foreground_label1_np)
                # np.save(label2_path, clusters_foreground_label2_np)
            ...
            #     cur_epe3d = torch.norm(pred_rigid_flows[h] - flow, dim=2).mean()
            #     print('{} : {}'.format(h, cur_epe3d))
            #     if cur_epe3d < epe3d:
            #         epe3d = cur_epe3d

            # epe3d = torch.norm(pred_rigid_flows[-1] - flow, dim=2).mean()
