...
class RandomSampling(Function):
    @staticmethod
    def forward(ctx, xyz: torch.Tensor, npoint: int) -> torch.Tensor:
        """
        Uses random point sampling to select a set of npoint
        :param ctx:
        :param xyz: (B, N, 3) where N > npoint
        :param npoint: int, number of features in the sampled set
        :return:
        ...
        random_indices_list = []
        for i in range(B):
            random_indices_list.append(torch.randperm(N))
        random_indices = torch.stack(random_indices_list, dim=0)
        output = random_indices[:,:npoint].to(device).contiguous().int()
        return output
...
random_sample = RandomSampling.apply
...
class knnQueryAndGroup(nn.Module):
    def __init__(self, nsample: int, use_xyz: bool = True):
        """
        :param radius: float, radius of ball
        :param nsample: int, the number of features to gather by using KNN
        :param use_xyz:
        """
        super().__init__()
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        :return:
            new_features: (B, 3 + C, npoint, nsample)
        """
        _, idx = knn(self.nsample, new_xyz, xyz) # idx (B, npoint, nsample)
        xyz_trans = xyz.transpose(1, 2).contiguous()
        grouped_xyz = grouping_operation(xyz_trans, idx)  # (B, 3, npoint, nsample)
        grouped_xyz -= new_xyz.transpose(1, 2).unsqueeze(-1)
