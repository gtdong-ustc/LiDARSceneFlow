...
class Resnet_BaseBlock(nn.Module):
    ...
    def forward(self, xyz, feats, new_xyz=None):
        ...
        # print('new_feats {}, xyz {}'.format(new_feats.shape, xyz.shape))
        # print('new_xyz {}'.format(new_xyz))
        # print(self.keep_pcd)
        new_xyz, new_feats, idx= self.conv2(xyz, new_feats, new_xyz)
...
class AssemRes_BaseBlock(nn.Module):
    ...
    def forward(self, xyz, feats, new_xyz=None):
        ...
        for i, block in enumerate(self.conv_blocks):
            # print(i)
            if i == 0:
                xyz, feats, idx = block(xyz, feats, new_xyz)
                # print('idx {}'.format(idx))
            else:
                xyz, feats, _ = block(xyz, feats, new_xyz)
        return xyz, feats, idx
