...
from networks.pointpwc import multiScaleChamferSmoothCurvature, ChamferSmoothCurvatureLoss
# from networks.pointconv_sf import NormLoss
# from networks.flownet3d_weakly_supervised_tmp import rigid_motion_consistency_loss
...
def main():
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        eval_epe3d, eval_loss, eval_chamfer_loss, eval_smoothness_loss, eval_curvature_loss, eval_ego_loss = eval_sceneflow(
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            # pos1, pos2, aligned_pos1, norm1, norm2, aligned_norm1, gt_rot, gt_trans, _ = data
            #
            # pos1 = pos1.cuda()
            # pos2 = pos2.cuda()
            # norm1 = norm1.cuda()
            ...
            if args.model_name == 'PointPWC':
                pre_aligned_pos1, pred_flows, _, _, pc1, pc2 = model(pos1, pos2, norm1, norm2, gt_label1, gt_label2)
                loss, chamfer_loss, curvature_loss, smoothness_loss = multiScaleChamferSmoothCurvature(pc1, pc2,pred_flows)
                diff_flow = pre_aligned_pos1.permute(0, 2, 1) - aligned_pos1
                ego_loss = 0.1 * torch.norm(diff_flow, dim=2).sum(dim=1).mean()
                loss += ego_loss
            elif args.model_name == 'FlowNet3D':
                pred_flow, weights, R_est, t_est = model(pos1, pos2, norm1, norm2, gt_label1, gt_label2)
                sf_loss, chamfer_loss, curvature_loss, smoothness_loss = ChamferSmoothCurvatureLoss(pos1, pos2, pred_flow, weights)
                pos2_est = transform_point_cloud(pos1, R_est, t_est)
                pred_ego_flow = pos2_est - pos1
                ego_loss = NormLoss(pos2_est, aligned_pos1)
                consistency_loss = rigid_motion_consistency_loss(pred_flow, pred_ego_flow, label1)
                loss = sf_loss + ego_loss + consistency_loss
        ...
        str_out4 = 'EPOCH %d %s mean ego_loss: %f' % (epoch, blue('train'), train_ego_loss)
        print(str_out1)
        print(str_out2)
        print(str_out3)
        print(str_out4)
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            eval_loss, chamfer_loss, curvature_loss, smoothness_loss = multiScaleChamferSmoothCurvature(pc1, pc2,
                                                                                                        pred_flows)
