...
from sample.sample import random_voxel_sample
...
class PointNetSetAbstraction_gz(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, sample_mode, voxel_size = 0.3):
        super(PointNetSetAbstraction_gz, self).__init__()
        self.npoint = npoint
        self.radius = radius
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all == False:
            ...
            elif self.sample_mode == 'RS':
                fps_idx = pointutils.random_sample(xyz_t, self.npoint) # [B, npoint]
                new_xyz = pointutils.gather_operation(xyz, fps_idx)  # [B, C, npoint]
            elif self.sample_mode == 'RVS':
                if self.npoint == 1024:
                    fps_idx = random_voxel_sample(xyz_t, self.npoint) # [B, npoint]
                else:
                    fps_idx = pointutils.random_sample(xyz_t, self.npoint)  # [B, npoint]
                new_xyz = pointutils.gather_operation(xyz, fps_idx)  # [B, C, npoint]
            else:
        ...
        new_points = self.queryandgroup(xyz_t, new_xyz.transpose(2, 1).contiguous(), points)  # [B, 3+C, N, S] S is nsamples
...
class PointNetFeaturePropogationSeg(nn.Module):
    def __init__(self, in_channel, mlp):
        super(PointNetFeaturePropogationSeg, self).__init__()
        self.mlp_convs = nn.ModuleList()
        self.mlp_bns = nn.ModuleList()
        last_channel = in_channel
        for out_channel in mlp:
