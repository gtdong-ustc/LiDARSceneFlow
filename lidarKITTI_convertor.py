...
def pc_loader(path):
    ...
    return pc1, pc2, aligned_pc1, flow, rot, trans, mot_label_1, mot_label_2, rel_trans, seg_label_1, seg_label_2

root_dir = '/home/gtdong/Documents/Dataset/WSSF/lidar_kitti_with_ground'

result_dir = '/home/gtdong/Documents/Dataset/WSSF/lidar_kitti_transfomer'
...
for file_name in file_list:
    ...
    res_file_path = osp.join(result_dir, file_name)
    pc1, pc2, aligned_pc1, flow, rot, trans, mot_label_1, mot_label_2, rel_trans, seg_label_1, seg_label_2 = pc_loader(file_path)
    ...
    # rel_trans[0:3, 0:3] = R_ref
    # rel_trans[0:3, 3:4] = t_ref
    ...
    # pcd_s_t.points = o3d.utility.Vector3dVector(pc1+process_flow)
    # pcd_s_t.paint_uniform_color([1.0, 0.0, 0.0])
    #
    # o3d.visualization.draw_geometries([pcd_s_t, pcd_t])
    np.savez(res_file_path, pc1=pc1, pc2=pc2, flow=process_flow, pose_s=rel_trans, sem_label_s=seg_label_1, sem_label_t=seg_label_2)

    fg_idx1 = np.where(mot_label_1[:, 0] == 1.0)[0]
    bg_idx1 = np.where(mot_label_1[:, 0] == 0.0)[0]
    print((mot_label_1-1).sum())
